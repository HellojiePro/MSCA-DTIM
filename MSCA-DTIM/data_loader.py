...
def load_tensor(file_name, dtype, device):
    ...
    if 'interactions' in file_name:
        data = np.array(data, dtype=np.float32)  # 强制转 float32
...
def train_data_load(dataset, device, DTI=True):
    ...
    sequence_train = np.load(dataset + '/train/sequences.npy', allow_pickle=True)
    smiles_train = np.load(dataset + '/train/smiles.npy', allow_pickle=True)
    if DTI == True:
...
def test_data_load(dataset, device, DTI=True):
    ...
    sequence_test = np.load(dataset + '/test/sequences.npy', allow_pickle=True)
    smiles_test = np.load(dataset + '/test/smiles.npy', allow_pickle=True)
    if DTI == True:
